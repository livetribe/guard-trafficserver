...
dnl
dnl LTG_CONFIG_NICE(filename)
dnl
dnl Saves a snapshot of the configure command-line for later reuse
dnl
AC_DEFUN([LTG_CONFIG_NICE], [
  rm -f $1
  cat >$1<<EOF
#! /bin/sh
#
...
dnl
dnl LTG_SETIFNULL(variable, value)
dnl
dnl  Set variable iff it's currently null
dnl
AC_DEFUN([LTG_SETIFNULL], [
  if test -z "$$1"; then
...
dnl
dnl LTG_REMOVEFROM(variable, value)
dnl
dnl Remove a value from a variable
dnl
AC_DEFUN([LTG_REMOVEFROM], [
...
dnl
dnl LTG_TRY_COMPILE_NO_WARNING(INCLUDES, FUNCTION-BODY,
...
dnl
AC_DEFUN([LTG_TRY_COMPILE_NO_WARNING],
...
dnl bar='${foo}/2'
dnl baz='${bar}/3'
dnl LTG_EXPAND_VAR(fraz, $baz)
dnl   $fraz is now "1/2/3"
dnl
AC_DEFUN([LTG_EXPAND_VAR], [
ats_last=
ats_cur="$2"
...
dnl LTG_PATH_RELATIVE(final_path, $orig_path, $prefix)
dnl    $final_path now contains "bar"
AC_DEFUN([LTG_PATH_RELATIVE], [
...
dnl LTG_SUBST(VARIABLE)
dnl Makes VARIABLE available in generated files
dnl (do not use @variable@ in Makefiles, but $(variable))
AC_DEFUN([LTG_SUBST], [
  LTG_VAR_SUBST="$LTG_VAR_SUBST $1"
  AC_SUBST($1)
])

dnl
dnl LTG_SUBST_LAYOUT_PATH
dnl Export (via LTG_SUBST) the various path-related variables that
dnl trafficserver will use while generating scripts and
dnl the default config file.
AC_DEFUN([LTG_SUBST_LAYOUT_PATH], [
  LTG_EXPAND_VAR(exp_$1, [$]$1)
  LTG_PATH_RELATIVE(rel_$1, [$]exp_$1, ${prefix})
  LTG_SUBST(exp_$1)
  LTG_SUBST(rel_$1)
  LTG_SUBST($1)
])

dnl LTG_HELP_STRING(LHS, RHS)
...
AC_DEFUN([LTG_HELP_STRING], [ifelse(regexp(AC_ACVERSION, 2\.1), -1, AC_HELP_STRING([$1],[$2]),[  ][$1] substr([                       ],len($1))[$2])])

dnl
dnl LTG_LAYOUT(configlayout, layoutname [, extravars])
dnl
AC_DEFUN([LTG_LAYOUT], [
...
dnl
dnl LTG_ENABLE_LAYOUT(default layout name [, extra vars])
dnl
AC_DEFUN([LTG_ENABLE_LAYOUT], [
AC_ARG_ENABLE(layout,
  [LTG_HELP_STRING([--enable-layout=LAYOUT],[Enable LAYOUT specified inside config.layout file (defaults to TrafficServer)])],[
  LAYOUT=$enableval
])
...
LTG_LAYOUT($srcdir/config.layout, $LAYOUT, $2)
...
dnl LTG_FLAG_HEADERS(header-file, [action-if-found], [action-if-not-found], [includes])
dnl
AC_DEFUN([LTG_FLAG_HEADERS], [
...
dnl
dnl LTG_SEARCH_LIBRARY(function, search-libs, [action-if-found], [action-if-not-found])
